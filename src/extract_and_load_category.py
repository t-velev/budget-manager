...
category_db_id = os.getenv('NOTION_DB_ID_CATEGORY')
...
new_data_filter = ['Name', 'Тип', 'Архивирай']  # A list of notion db column names to be filtered. Empty list filters nothing.
...
upsert_into_stats(engine, len(category_new_data), run_id, run_date, dag_name, task_name, column='ntn_extracted')

# Extract and name only the needed columns
new_data = []

for i, item in enumerate(category_new_data):
    new_data.append(
         {
          'id':                item['id']                                                                                            ,
