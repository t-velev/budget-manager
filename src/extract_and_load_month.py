...
month_db_id = os.getenv('NOTION_DB_ID_MONTH')
...
task_name = os.getenv('task_name', 'extract_and_load_month')
...
new_data_filter = ['Name']  # A list of notion db column names to be filtered. Empty list filters nothing.

# Extract ONLY NEW data, no filters
month_new_data = get_data(month_db_id, last_load_date, filter_cols=new_data_filter)

# For testing purposes during development
# with open('./data/notion_month_extract.json', 'r', encoding='utf-8') as file:
#   month_new_data = json.load(file)

print(f'Extracted {len(month_new_data)} new rows from Notion.')

# Write the extracted count to sys_etl_stats table
upsert_into_stats(engine, len(month_new_data), run_id, run_date, dag_name, task_name, column='ntn_extracted')

# Extract and name only the needed columns
new_data = []

for i, item in enumerate(month_new_data):
    new_data.append(
         {
          'id':                item['id']                                                                                            ,
...
filtered_data = get_data(month_db_id, last_load_date=None, filter_cols=id_cols_filter)
